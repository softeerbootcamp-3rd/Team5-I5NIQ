...
public class Listener {
  ...
  public void accessAllow(int clientNumber){
    int nowCount=-1;
    logger.info("[Listener] access allow for {}, socket count : {}",clientNumber,socketChannelQueue.size());
    for(int i=0;i<clientNumber;i++){
      ...
      nowCount = ticketCount.incrementAndGet();
    ...
    broadCast(buffer);
    logger.info("ticket count : {}",ticketCount.get());
  }
